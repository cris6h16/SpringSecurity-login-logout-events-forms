...
//                        .defaultSuccessUrl("/", false)        // ( path, alwaysUse )
		...
		/* Default logout:
			1. when you use `@EnableWebSecurity` Spring add its logout support 
			   By Default 
				if CSRF is enable:
					1. GET  /logout => displays confimation page 
					2.  POST /logout => performs the logout passing the _csrf
				if CSRF is disable:
					1. GET logout => perform logout directly 
